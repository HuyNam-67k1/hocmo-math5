/* eslint-disable no-undef */
// 🔴 DO NOT EDIT — This file is generated.
import * as React from 'react';
import { SVGProps } from 'react';
const ReportActiveSvg = (props: SVGProps<SVGSVGElement>) => (
  <svg
    width="1em"
    height="1em"
    viewBox="0 0 20 20"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    data-custo="fill"
    {...props}
  >
    <path
      d="M18.3346 18.3333H1.66797C1.3263 18.3333 1.04297 18.0499 1.04297 17.7083C1.04297 17.3666 1.3263 17.0833 1.66797 17.0833H18.3346C18.6763 17.0833 18.9596 17.3666 18.9596 17.7083C18.9596 18.0499 18.6763 18.3333 18.3346 18.3333Z"
      fill="#219B67"
    />
    <path
      d="M8.125 3.33341V18.3334H11.875V3.33341C11.875 2.41675 11.5 1.66675 10.375 1.66675H9.625C8.5 1.66675 8.125 2.41675 8.125 3.33341Z"
      fill="#219B67"
    />
    <path
      d="M2.5 8.33341V18.3334H5.83333V8.33341C5.83333 7.41675 5.5 6.66675 4.5 6.66675H3.83333C2.83333 6.66675 2.5 7.41675 2.5 8.33341Z"
      fill="#219B67"
    />
    <path
      d="M14.168 12.4999V18.3333H17.5013V12.4999C17.5013 11.5833 17.168 10.8333 16.168 10.8333H15.5013C14.5013 10.8333 14.168 11.5833 14.168 12.4999Z"
      fill="#219B67"
    />
  </svg>
);
export default ReportActiveSvg;
